...
I used this [guide](https://developers.google.com/web/fundamentals/push-notifications)⧉ to get a better understanding of implementing push notifications. I highly recommend atleast skimming through that as building out a push notification system involves an end-to-end understanding of the process.

> This post is not a step-by-step guide and assumes comfort with a Gatsby project, installing npm packages and so on. For a full look-through of the code, there's a link to this site's source code at the [end of the page](#references). 
...
> Update (June-06-2020): My opinion of using a serverless database has significantly changed since implementing this. FaunaDB is definitely still a viable choice, but I think using Google's Firestore (which is also truly free **and** serverless) will be a much better experience especially when it comes to writing queries and making calls to the Firestore endpoint. That being said, I still continue to use FaunaDB because it still works and the effort of migration isn't worth the benefit, which at this stage, is actually nothing. So, if you're curious, or just want to use FaunaDB, continue with this section. 

---

I chose FaunaDB because it was one of the few serverless providers I could find that I knew would be truly free without resorting to dynamo or Firebase (not that there’s anything wrong with them I just didn't want to set it up).
...
- Code is public, you can view the full source [here](https://github.com/dshomoye/dshomoye.github.io) - and feel free to star/follow or drop a note. I'd love to know if/how I've helped anyone who was as lost as me.
